// Per-project Jest config for user-management (SPEC §11.1 — tests live in a per-project `test/`
// folder mirroring `src/`). Resolves @aegis/* path aliases so tests can import across libs, and only
// picks up specs under this project's `test/` directory. Runnable both via the nx `test` target
// (`nx test user-management`) and the root jest (`npx jest apps/user-management`).
