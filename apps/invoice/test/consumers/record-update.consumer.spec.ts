/**
 * BUG-0003 — RecordUpdated consumer (invoice half). The workflow `assign_team` / `add_tag` actions
 ...
 * annotation to the invoice's idempotent `applyRecordUpdate`. Tests: an invoice RecordUpdated
...
it('applies team + tags for an invoice RecordUpdated', async () => {
  const onRecordUpdated = handler();
  await withTenant('t1', () => onRecordUpdated(envelope()));
  expect(applyRecordUpdate).toHaveBeenCalledWith(INVOICE_ID, {
    teamId: 'team-9',
    tags: ['urgent', 'q3'],
    ruleId: 'rule-tag',
  });
...
it('ignores RecordUpdated for a non-invoice record type', async () => {
  const onRecordUpdated = handler();
  await withTenant('t1', () =>
    onRecordUpdated(envelope({ recordType: ApprovalRecordType.ExpenseReport })),
  );
