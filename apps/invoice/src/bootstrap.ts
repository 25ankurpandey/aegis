...
/**
 * Composition root for invoice (the donor bootstrap pattern). Defines the invoice models on the
 * shared connection, registers the built-in ERP connectors (the approved-invoice push target), loads
 * the DI providers (controllers + service + repository), then forks on `PROCESS_TYPE`:
 *
 ...
 *    consumers. No HTTP server — this half advances stranded invoices from the relayed event.
...
async function init(): Promise<void> {
  getInvoiceContext(); // define invoice models on the shared connection
  registerBuiltinConnectors(); // make the mock ERP connectors available for approved-invoice push
  loadProviders(); // bind controllers + service + repository into the container
  ...
  if (Config.get('PROCESS_TYPE') === 'worker') {
    ...
    if (isKafkaBus()) {
      ...
      Logger.info('invoice worker running (kafka consumers)');
    } else {
      Logger.info('invoice worker running (in-process bus; no KAFKA_BROKERS)');
    }
    return;
  }
  ...
  service.start(Config.int('PORT', 4007));
}
