 ...
 *
 * REUSABLE APPROVAL-ENGINE WIRING (the template invoice/payroll copy): `registerApprovalProviders()`
 ...
 * alongside its own providers. The service then just `@inject(ApprovalService)`. Do NOT load the
 * provider module a second time for approvals — the global registry means one load already covers it.
 */
