/**
 * BUG-0003 — RecordUpdated consumer (expense half). The workflow `assign_team` / `add_tag` actions
 ...
 * annotation to the report's idempotent `applyRecordUpdate`. Tests: an expense_report RecordUpdated
...
it('applies team + tags for an expense_report RecordUpdated', async () => {
  const onRecordUpdated = handler();
  await withTenant('t1', () => onRecordUpdated(envelope()));
  expect(applyRecordUpdate).toHaveBeenCalledWith(REPORT_ID, {
    teamId: 'team-9',
    tags: ['urgent', 'q3'],
    ruleId: 'rule-tag',
  });
...
it('ignores RecordUpdated for a non-expense record type', async () => {
  const onRecordUpdated = handler();
  await withTenant('t1', () =>
