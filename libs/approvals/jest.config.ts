// Per-project Jest config for the @aegis/approvals lib. Resolves @aegis/* path aliases so specs can
// import across libs under ts-jest, and picks up the engine specs under a `test/` folder
// mirroring `src/`. Runnable
// both via the nx `test` target (`nx test approvals`) and the root jest (`npx jest libs/approvals`).
