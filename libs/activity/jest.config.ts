// Per-project Jest config for the @aegis/activity lib. Resolves @aegis/* path aliases so specs can
// import across libs under ts-jest, and picks up the logger specs under a `test/` folder
// mirroring `src/`. Runnable
// both via the nx `test` target (`nx test activity`) and the root jest (`npx jest libs/activity`).
