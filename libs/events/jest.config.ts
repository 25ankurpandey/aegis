// Per-project Jest config for the @aegis/events lib. Resolves @aegis/* path aliases so specs can
// import across libs under ts-jest, and picks up the bus/kafka-bus/outbox/payloads specs under a
// `test/` folder mirroring `src/`. Runnable both via the nx `test` target (`nx test events`) and the
// root jest (`npx jest libs/events`).
