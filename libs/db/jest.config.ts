// Per-project Jest config for the @aegis/db lib. Resolves @aegis/* path aliases so specs can import
// across libs under ts-jest, and picks up the model-registry + feature-flags-reader specs under a
// `test/` folder mirroring `src/`. Runnable both via the nx `test` target (`nx test db`) and the
// root jest (`npx jest libs/db`).
