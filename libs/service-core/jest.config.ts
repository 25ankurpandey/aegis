// Per-project Jest config for the @aegis/service-core lib. Resolves @aegis/* path aliases so specs
// can import across libs under ts-jest, and picks up the bootstrap/cache/config/http/logging/
// middleware specs under a `test/` folder mirroring `src/`. Runnable both via the nx `test` target
// (`nx test service-core`) and the root jest (`npx jest libs/service-core`).
